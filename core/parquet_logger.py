...
class ParquetLogger:
    ...
    def __init__(self, filepath: str, flush_size: int = 100,
        ...
        self._pg         = pg_backend
        # Derive table name from the file stem when not provided explicitly
        self._table_name = table_name or os.path.splitext(
            os.path.basename(filepath)
        )[0]
        os.makedirs(os.path.dirname(os.path.abspath(filepath)), exist_ok=True)

        # On cold start (no local file) seed from PostgreSQL so we don't lose
        # experience history across Railway redeployments.
